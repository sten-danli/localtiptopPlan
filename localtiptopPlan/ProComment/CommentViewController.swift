...
class CommentViewController: UIViewController {
    ...
    // MARK: -var / let
    var comments = [CommentModel]()//把 loadComments() 取出的comments存入到这里。
    var users = [User]()
...
// MARK：-Creat Comment
    func creatComment(){
        ...
        let dic = ["uid": uid, "commentText": commenTextField.text!,
                   "conproTag": conproTag] as [String : Any]
...
// MARK: - load comments
    ...
    func loadComments() {
        ...
        let postCommentRef = Database.database().reference().child("postComments").child(postId)//并取出当前postId下所有的commentsID
     
        postCommentRef.observe(.childAdded) { (snapshot) in//存入到snapshot中，
            let commentRef = Database.database().reference().child("comments").child(snapshot.key)//然后在firebase “comments”中把对应commentsID的内容取出来，
            commentRef.observeSingleEvent(of: .value, with: { (snapshot) in//放入snapshot里面，
                ...
                let newComment = CommentModel(dictionary: dic)//dic用CommentModel解释并存入newComment中。
